...
function build_data_table(result){
	...
	$.each(data,function(index,item){
		...
		*/
		...
		var heart = $("<i></i>").addClass("am-icon-heart-o").addClass(" heartMouse ");
		...
		var comment = $("<i></i>").addClass("am-icon-comment");
		var ahuifuDom = $("<a></a>").addClass(" huifu").attr("title","[回复]").append("[回复]").attr("id",item.id).append("（").append(pinglun).append("）");
		...
		$.each(children,function(index,child){
			...
			var headDiv = $("<div></div>").addClass("headUrl-huifu").append(imgHfDom).append(headTitle).append(azanHfDom).append(likes).append(ahuifuDom2);
...
$("body").on('click', '.huifu', function () {
	...
	var name = $(this).prev().prev().prev().find("span:first").text();
...
$("#blog-nav li a[href=say]").addClass("active");
$("#blog-nav li a .active ").parent().css("color","red");
...
$(".touxiang-list ul li").click(function () {
	...
	$(".check-toux img").attr("src" ,simg.attr("src"));
});
...
$(".message").focus(function(){
    ...
    $(".message").css("color","black");//transparent
});
$(".message").blur(function(){
	if($(".message").html()==''){
		$(".message").html("来都来了，不说点什么吗？");
	}
	
	$(".message").css("color","transparent");//transparent
});
...
function addEvent(cstr){
	...
	//----------------------------------------------------------------
	//点击小图片，显示表情
	$(".bq-touxiang"+cstr).click(function (e) {
		$(".touxiang-list"+cstr).slideDown(); //慢慢向下展开
		e.stopPropagation(); //阻止冒泡事件
	});

	//在桌面任意地方点击，关闭表情框
	$(document).click(function () {
		$(".touxiang-list"+cstr).slideUp(); //慢慢向上收
	});
	
	$(".check-toux"+cstr+" img").click(function(){
	...
	//点击小图标时，添加功能
	$(".touxiang-list"+cstr+" ul li").click(function () {
		let simg = $(this).find("img").clone();
		$("#utx"+cstr).val(simg.attr("src"));
		$(".check-toux"+cstr+" img").attr("src" ,simg.attr("src"));
	});

	$(".touxiang-list"+cstr+" li").mouseover(function(){
		let simg = $(this).find("img").clone();
		simg.attr("width","").attr("height","");
		
		$(".big-toux"+cstr+" img").attr("src",simg.attr("src"));
		$(".big-toux"+cstr).css({"display": "block"});
	});

	$(".touxiang-list"+cstr+" li").mouseout(function(){
		$(".big-toux"+cstr).css({"display":"none"});
	});
	
	$(".huifu-message"+cstr).focus(function(){
		console.log(at);
		if(at==undefined || at == ''){
			at =  $(".huifu-message"+cstr).html();
			$(".huifu-message"+cstr).html('');
		}
		
	    $(".huifu-message"+cstr).css("color","black");//transparent
	});
	$(".huifu-message"+cstr).blur(function(){
		if($(".huifu-message"+cstr).html()==''){
			$(".huifu-message"+cstr).html(at);
		}
		$(".huifu-message"+cstr).css("color","transparent");//transparent
	});
}
		...
		`<div class="tmp-hui-fu" id="qq-1" style="boder-color:black;">
		  <form id="say-1" class="form-horizontal" onsubmit="return false"> 
		   ...
		   <div class="form-group input-group-sm">
				<div class="col-sm-1  "> 
				<div class="check-toux${isparent}"><img src="moudle/say/images/tx_kt_7.jpg" width="33px" height="33px" class="bq-touxiang${isparent} "></div>
				<div class="touxiang-list${isparent}">
				<div class="big-toux${isparent}"><img /></div> 
			     <ul> 
